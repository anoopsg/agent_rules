...
OPTIONS
  -t, --targets <list>  Comma-separated agents (default: all).
                        Values: antigravity (ag), claude (cl), cursor (cu), kiro (kr)
...
generate_claude() {
  ...
  bash "$SCRIPT_DIR/_agents/claude.sh" \
    "$RULES_DIR" \
    "$SKILLS_DIR" \
    "$OUTPUT_DIR" \
    "$exclusive_path" \
    "$GEN_CLEAN" >/dev/null
  gen_status "Claude" "$OUTPUT_DIR/.claude"
}
